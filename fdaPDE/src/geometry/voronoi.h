...
// Import the headers from the rest of fdaPDE
...
namespace fdapde {
  ...
  // This is a basic (not working) structure for the surface Voronoi
  // Actually only defined some general routines that are needed in the Depth computation
  // The core method, that is the measure() of a Voronoi cell, is demanded to external computations with respect to this library
  template <> class Voronoi<Triangulation<3, 3>> {
  public:
    static constexpr int local_dim = Triangulation<3,3>::local_dim;
    static constexpr int embed_dim = Triangulation<3,3>::embed_dim;

    Voronoi() = default;
    Voronoi(const Triangulation<3, 3>& mesh) : mesh_(&mesh) {   // constructs voronoi diagram from Delanoy triangulation // For the moment it is just empty
    ...
    public:
      ...
      Eigen::Matrix<int, Dynamic, Dynamic> edges() const {
	// This is a fake utility, at the moment is not working
	Eigen::Matrix<int, Dynamic, Dynamic> result;
	return result;
      }
	...
	// For the moment this is taylor made only for the sphere and in practice is not used i the upstream code. Keep in mind that needs to be modified when dealing with more complex data
	return false;   // no edge on boundary
      }
      bool contains(const Eigen::Matrix<double,embed_dim,1>& p) const { return v_->locate(p)[0] == id_; }
    };
    // getters
    const Eigen::Matrix<double, Dynamic, Dynamic>& sites() const { return mesh_->nodes(); }
    Eigen::Matrix<double,embed_dim,1> site(int id) const { return mesh_->node(id); }
    const BinaryVector<fdapde::Dynamic>& boundary_vertices() const { return nodes_markers_; }
    const Eigen::Matrix<double, Dynamic, Dynamic>& vertices() const { return nodes_; }
    ...
    // perform point location for set of points p_1, p_2, p_3, \ldots, p_n
    Eigen::Matrix<int, Dynamic, 1>  locate(const Eigen::Matrix<double, Dynamic, Dynamic>& locs) const {
      ...
      Eigen::Matrix<int, Dynamic, 1>  dual_locs = mesh_->locate(locs);
	...
	Eigen::Matrix<double,1, Triangulation<3, 3>::n_nodes_per_cell> dist =
	  (f.nodes().colwise() - locs.row(i).transpose()).colwise().squaredNorm();
	int min_index;
	double min = dist.minCoeff(&min_index);
	Eigen::Matrix<int, Dynamic, 1>  neighbors = f.neighbors(); // Remark: This works great only for a sphere. In a non convex manifold it would be wrong
	for(int j = 0; j<neighbors.size(); ++j){ // First level neighbors
	  typename Triangulation<3, 3>::CellType f_1 = mesh_->cell(neighbors[j]);
	  Eigen::Matrix<double,1,Triangulation<3, 3>::n_nodes_per_cell> dist_1 =
  ...
  private:
    ...
    Eigen::Matrix<double, Dynamic, Dynamic> nodes_;                             // voronoi vertices
  ...
  public:
	...
	Eigen::Matrix<double,1, Triangulation<2, 3>::n_nodes_per_cell> dist =
	  (f.nodes().colwise() - locs.row(i).transpose()).colwise().squaredNorm();
	int min_index;
	double min = dist.minCoeff(&min_index);
	Eigen::Matrix<int, Dynamic, 1>  neighbors = f.neighbors(); // Remark: This works great only for a sphere. In a non convex manifold it would be wrong
	for(int j = 0; j<neighbors.size(); ++j){ // First level neighbors
	  ...
	  Eigen::Matrix<double,1, Triangulation<2, 3>::n_nodes_per_cell> dist_1 =
	    (f_1.nodes().colwise() - locs.row(i).transpose()).colwise().squaredNorm();
	  int min_index_1;
	  double min_1 = dist_1.minCoeff(&min_index_1);
	  if(min_1<min){
  ...
  public:
	...
	Eigen::Matrix<double,embed_dim,1> mean = Eigen::Matrix<double,embed_dim,1>::Zero();
	auto compare = internals::clockwise_order<Eigen::Matrix<double,embed_dim,1>>(
										     std::accumulate(
	...
	for (int j = 0; j < n_edges_; ++j) {
	  ...
	  Eigen::Matrix<double,embed_dim,1> x = v_->vertex(v_->cells_.at(id_)[j]);
	  Eigen::Matrix<double,embed_dim,1> y = v_->vertex(v_->cells_.at(id_)[(j + 1) % n_edges_]);
	...
	Eigen::Matrix<double,embed_dim, 2> coords;
    ...
    // getters
    const Eigen::Matrix<double, Dynamic, Dynamic>& sites() const { return mesh_->nodes(); }
    Eigen::Matrix<double,embed_dim,1> vertex(int id) const { return nodes_.row(id); }
    Eigen::Matrix<double,embed_dim,1> site(int id) const { return mesh_->node(id); }
    const BinaryVector<fdapde::Dynamic>& boundary_vertices() const { return nodes_markers_; }
    const Eigen::Matrix<double, Dynamic, Dynamic>& vertices() const { return nodes_; }
    ...
    // compute matrix of edges
    Eigen::Matrix<int, Dynamic, Dynamic> edges() const {
      std::unordered_set<std::array<int, local_dim>, internals::std_array_hash<int, local_dim>> visited;
	...
	Eigen::Matrix<double,1, Triangulation<2, 2>::n_nodes_per_cell> dist =
	  (f.nodes().colwise() - locs.row(i).transpose()).colwise().squaredNorm();
	int min_index;
	double min = dist.minCoeff(&min_index);
	Eigen::Matrix<int, Dynamic, 1>  neighbors = f.neighbors();
	for(int j = 0; j<neighbors.size(); ++j){ // First level neighbors
	  typename Triangulation<2, 2>::CellType f_1 = mesh_->cell(neighbors[j]);
	  Eigen::Matrix<double,1, Triangulation<2, 2>::n_nodes_per_cell> dist_1 =
	    (f_1.nodes().colwise() - locs.row(i).transpose()).colwise().squaredNorm();
	  int min_index_1;
	  double min_1 = dist_1.minCoeff(&min_index_1);
	  if(min_1<min){
  ...
  // Missing the case <1,2>, that in principle is quite easy to be handled, but for the moment never arised in practice
  ...
  public:
    ...
    // getters
    const Eigen::Matrix<double, Dynamic, 1>& sites() const { return mesh_->nodes(); }
    Eigen::Matrix<double,embed_dim,1> vertex(int id) const { return nodes_.row(id); }
    Eigen::Matrix<double,embed_dim,1> site(int id) const { return mesh_->node(id); }
    const BinaryVector<fdapde::Dynamic>& boundary_vertices() const { return nodes_markers_; }
    const Eigen::Matrix<double, Dynamic, Dynamic>& vertices() const { return nodes_; }
	...
	Eigen::Matrix<double,1, Triangulation<1, 1>::n_nodes_per_cell> dist =
	  (f.nodes().colwise() - locs.row(i).transpose()).colwise().squaredNorm();
	int min_index;
